...
static volatile uint32_t g_buffer[BUFFER_SIZE];
...
void core1_main()
{
    while (1)
    {
        if (multicore_fifo_rvalid())
        {
            ...
            gpio_init(TRIGGER_INDEX);
            gpio_set_dir(TRIGGER_INDEX, GPIO_IN);
            gpio_pull_up(TRIGGER_INDEX);
            ...
            while (1)
            {
                ...
                {
                    if (index < (sizeof(g_buffer) - 4)) // Reserve the space for a possible reset
                    {
                        g_buffer[index++] = v;
                    }
                    else
